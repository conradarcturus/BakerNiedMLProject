  ...
  for date in dates:
    ...
    try:
      ...
      for trip_id in trip_ids.astype(str):
        ...
        temp = (data[:, 18] == trip_id) & (data[:, 8] == "NULL")
        temp2 = data[temp,:];
        ...
        N_bin     = int(math.ceil(dist_max / 100))
        ...
        for i in range(N_bin):
          ...
          #print data_sorted[loc+1,DIST]-di;
          #print len(data_sorted)-2;
          #print data_sorted[loc+1,DIST]
          while(data_sorted[loc+1,DIST]-di<0) and loc<len(data_sorted)-2 and data_sorted[loc+1,DIST] < tripDist:
            loc+=1;
            #print "loc is "+str(loc)
          deltaTime = (data_sorted[loc+1,DEV]-data_sorted[loc,DEV])/60;
          #print deltaTime
          deltaDist = data_sorted[loc+1,DIST]-data_sorted[loc,DIST];
          #print deltaDist
          val = data_sorted[loc,DEV]+math.floor(deltaTime*(di-data_sorted[loc,DIST])/deltaDist)*60;
          #print val
          newTraj[i] = val;
        data_out.append(newTraj.copy());
      code.interact(local=locals())
      ...
      #shape_dist = np.sum((np.tile(shape_int[:, 1:].reshape(N_bins, 2, 1), (1, 1, N)) - np.tile(np.transpose(data_sorted[:, 1:2].reshape(N, 2, 1), (2, 1, 0)), (N_bins, 1, 1))) ** 2), axis=1)
      #data_newdist = shape_int[dist.argsort(axis=0)[:, 1]]
      #data_newdist = kNN.manyNearestNeighborsVector(shape_int[:, 1:2], shape_int[:, 0], data_sorted[:, 1:2], 1)

      
      
      # Save Data
      fileout = "/projects/onebusaway/BakerNiedMLProject/data/routedays/intercitytransit_route13_traj{}.txt".format(date);
