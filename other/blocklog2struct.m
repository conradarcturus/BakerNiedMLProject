function data = blocklog2struct(filename)
% Loads a CSV file to a structure
% 
% Author: A. Conrad Nied (anied@cs.washington.edu)
%
% Changelog:
% 2013-11-05 Created, based on csv2struct
...
%         fprintf('%d\t%s\t%s', N_entries, field, entry);
        if(strcmp(entry, 'NULL'))
            entryline.(field) = [];
        elseif(mean(isstrprop(entry, 'digit') | isstrprop(entry, 'punct'))==1)
